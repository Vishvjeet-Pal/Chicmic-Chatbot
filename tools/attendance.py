...
def register_attendance(mcp):
    @mcp.tool()
    async def get_daily_attendence(auth_token,request_data,date=datetime.today().strftime('%d-%m-%Y')):
        """
...
Note: This tool does not provide the office timing of the user. It only tells the timing when user came to office/workzone. Office timing may be different from main gate/office in time of user. 
    ...
    f"You were {'PRESENT' if attendance.get('guardInTime') else 'ABSENT'} on date {datetime.fromisoformat(attendance.get('date').replace('Z','+00:00')).strftime('%d %B %Y')} or {datetime.fromisoformat(attendance.get('date').replace('Z','+00:00')).strftime('%d-%m-%Y')} or {datetime.fromisoformat(attendance.get('date').replace('Z','+00:00')).strftime('%d %b')} \n"
    f"Your work zone in Time: {datetime.strptime(attendance.get('inTime'), '%Y-%m-%dT%H:%M:%S.%fZ').replace(tzinfo=timezone.utc).astimezone(IST).strftime('%I:%M %p') if attendance.get('inTime') else 'Not mentioned in erp'}\n"
    f"Your work zone Out Time: {datetime.strptime(attendance.get('outTime'), '%Y-%m-%dT%H:%M:%S.%fZ').replace(tzinfo=timezone.utc).astimezone(IST).strftime('%I:%M %p') if attendance.get('outTime') else 'Not mentioned in erp'}\n"
    ...
    for attendance in attendance_data if datetime.fromisoformat(attendance.get('date').replace('Z','+00:00')).strftime('%d-%m-%Y')==final_date
])
